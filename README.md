# AkStackPro Shipping Label Integration
...
- Magento 2.4.x with `Magento_Shipping` enabled
- PHP 8.1+ (Magento 2.4.7 requirement)
- **Store Information** configured in Magento (used as ship-from address):
  - **Stores → Configuration → General → General → Store Information**
  - Store Name, Street Address, City, State/Region, ZIP, Phone
- Valid API credentials for the carrier(s) you plan to use
- Orders must be **invoiced** before a shipment can be created (Magento + module requirement)

---

## Installation

1. Copy the module into your Magento `app/code` directory:

   ```
   app/code/AkStackPro/ShippingLabelIntegration/
   ```

2. Enable the module and run setup:

   ```bash
   bin/magento module:enable AkStackPro_ShippingLabelIntegration
   bin/magento setup:upgrade
   bin/magento setup:di:compile
   bin/magento cache:flush
   ```

3. Assign the configuration ACL resource to your Admin role (if ACL is enforced in your environment):
   - Resource: `AkStackPro_ShippingLabelIntegration::configuration`
...
**Stores → Configuration → Ak Stack Pro → UPS And Endicia Web Services**
...
| **Debug** | When enabled, API request/response details are written to `var/log/system.log`. |

### Endicia Shipping Integration Settings
...
### UPS Worldship Shipping Integration Settings
...
1. Go to **Stores → Configuration → Ak Stack Pro → Developer Section**
...
For issues, enable **Debug** mode, reproduce the problem, and review `var/log/system.log` for API request/response details before contacting support.
